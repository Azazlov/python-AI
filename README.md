...
Предоставление прав на доступ в локальную сеть для Python **не обязательно**. Это требуется только в случае подключения к хостингу с других компьютеров по локальной сети.